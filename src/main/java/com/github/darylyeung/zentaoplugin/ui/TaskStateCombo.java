package com.github.darylyeung.zentaoplugin.ui;

import com.github.darylyeung.zentaoplugin.util.TaskUiUtil;
...
public abstract class TaskStateCombo extends JPanel {
  ...
  public boolean scheduleUpdateOnce() {
    if (myProject != null && stateUpdatesSupportedFor(myTask) && myKindCombo.getComboBox().getItemCount() == 0) {
      ...
      new TaskUiUtil.ComboBoxUpdater<CustomStateTrinityAdapter>(myProject, TaskBundle.message("progress.title.fetching.available.task.states"), comboBox) {
        @NotNull
        @Override
